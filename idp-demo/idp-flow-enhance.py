...
async def apply_doc_rules(document_content, user_prompt):
    summary_model = "vllm-llama-3.2-vision" #"vllm-llama-3.3"
    langfuse_context.update_current_trace(
        metadata={"run_id": run_id}
    )
...
async def extraction_node(state: State) -> State:
    state["messages"].append(AIMessage(content=await extract_doc("se.png", se_document_user_prompt), role="assistant"))
    return state

async def rule_node(state: State) -> State:
    state["messages"].append(AIMessage(content=await apply_doc_rules(state["messages"][0].content, se_document_rule_prompt), additional_kwargs={"role": "assistant"}))
    return state
...
def auto_process(state: State) -> State:
    ...
    print(f"Auto Process Message is: {auto_process_message}")
